...
class Create extends JsonApiController
{
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        $attendanceSessionId = self::arrayGet($json, 'data.attributes.attendance-session-id');

        $session = AttendanceSession::find($attendanceSessionId);
        if (!$session) {
    ...
    protected function validateResourceDocument($json, $data)
    {
        ...
        if (!self::arrayHas($json, 'data.attributes.attendance-session-id')) {
            return 'Missing `attendance-session-id` member of attributes block.';
        }
    ...
    /**
     * Extract data and creates entry.
     * @param array $json
     * @return AttendanceEntry
     */
    private function createEntry(array $json)
    {
        // Required fields.
        $attendanceSessionId = (int) self::arrayGet($json, 'data.attributes.attendance-session-id');
        ...
        $entry->attendance_session_id = $attendanceSessionId;
